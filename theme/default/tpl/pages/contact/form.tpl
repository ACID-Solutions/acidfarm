...
<div id="contact_inputs" class="contact_block">
    ...
    <div class="input row">
        <div class="label"><label for="input_firstname"><?php echo Acid::trad('contact_form_firstname'); ?></label>
        </div>
    ...
    <?php if ($rgpd_notice = $g['site_config']->getConf('rgpd_notice_' . Acid::get('lang:current'))) { ?>
        <div class="input row">
            <div id="contact_rgpd_notice">
                <div>
                    <input type="checkbox" name="<?php echo 'rgpd_notice_' . Acid::get('lang:current'); ?>" value="1"/>
                </div>
                <div class="rgpd_notice" onclick="$('#contact_rgpd_notice [type=checkbox]').click();">
                    <?php echo $rgpd_notice; ?>
                </div>
            </div>
        </div>
    <?php } ?>
    ...
    <div class="row">
        <?php if (Conf::get('contact:shield')) { ?>
            <noscript>
                <div style="color:red;"><?php echo Acid::trad('contact_please_enable_javascript'); ?></div>
            </noscript>
        <?php } ?>
...
<script type="text/javascript">
    ...
    $().ready(function() {
        ...
        setTimeout(function() {
            <?php }  ?>
            $('#contact_form').append(
                '<input type="hidden" name="<?php echo Conf::get('contact:shield_key'); ?>"' +
                ' value="<?php echo Conf::get('contact:shield_val'); ?>"  />');
            <?php if (Conf::get('contact:shield_time')) { ?>
