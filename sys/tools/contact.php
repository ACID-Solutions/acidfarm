...
class Contact
{
    ...
    public static function exePost()
    {
        if (isset($_POST['contact_do'])) {
            ...
            if ($do == 'send_form') {
                if ((!Conf::get('contact:shield'))
                    || (Lib::getInPost(Conf::get('contact:shield_key'), 'novalue')
                        == Conf::get('contact:shield_val'))) {
                    Contact::exeForm();
                } else {
    ...
    {
        $def_excluded = ['contact_do', 'module_do', 'x', 'y', 'g-recaptcha-response'];
    ...
    {
        $def_tab = [
            'phone'   => ['label' => Acid::trad('contact_post_phone'), 'regex' => ''],
            'address' => ['label' => Acid::trad('contact_post_address'), 'regex' => ''],
            
            'cause_1'        => ['label' => Acid::trad('contact_post_cause_1'), 'regex' => ''],
            'cause_2'        => ['label' => Acid::trad('contact_post_cause_2'), 'regex' => ''],
            'cause_3'        => ['label' => Acid::trad('contact_post_cause_3'), 'regex' => ''],
            'get_newsletter' => ['label' => Acid::trad('contact_post_get_newsletter'), 'regex' => '']
    ...
    {
        $def_tab = [
            'message'   => ['label' => Acid::trad('contact_post_message'), 'regex' => ''],
            'lastname'  => ['label' => Acid::trad('contact_post_lastname'), 'regex' => ''],
            'firstname' => ['label' => Acid::trad('contact_post_firstname'), 'regex' => ''],
            'email'     => [
    ...
    {
        return Acid::tpl('pages/contact.tpl', []);
    }
    ...
    public static function exeForm()
    {
        ...
        //rgpd case
        $rgpd_notice_key = 'rgpd_notice_' . Acid::get('lang:current');
        if ($rgpd_notice = SiteConfig::getCurrent()->getConf($rgpd_notice_key)) {
            $form_control_keys[$rgpd_notice_key] = [
                'label' => Acid::trad('contact_post_rgpd_notice'),
                'regex' => ''
            ];
            $_POST[$rgpd_notice_key] = (!empty($_POST[$rgpd_notice_key]) ? $rgpd_notice : '');
        }
        ...
        if (empty($miss) && empty($mistake)) {
            // CONFIGURE BODY
            //email configuration
            $from_name = Acid::trad('contact_post_form_of', ['__SITE__' => Acid::get('site:name')]);
            $from_email =
                Conf::exists('contact_form:sender') ? Conf::get('contact_form:sender') : Acid::get('site:email');
            $to_email = $GLOBALS['site_config']->getConf('email');
            $subject = Acid::trad('contact_post_form_of_subject', ['__SITE__' => Acid::get('site:name')]);
            ...
            foreach ($add as $key => $val) {
                if (strpos($key, 'cause_') === 0) {
                    ...
                    if ($key == 'message') {
                        $message_val[(!empty($form_controler[$key]['label']) ? $form_controler[$key]['label'] : $key)] =
                            $val;
                    } else {
                        $profile[(!empty($form_controler[$key]['label']) ? $form_controler[$key]['label'] : $key)] =
                            $val;
                    }
                }
            }
            ...
            $body = Acid::tpl('admin/contact-mail.tpl',
                ['cause' => $cause, 'profile' => $profile, 'message' => $message_val]);
            ...
            if ($reply_mail) {
                ...
                $function_tab['AddReplyTo'] = [$reply_mail, utf8_decode($reply_name)];
            }
            
            //recipient address defined
            if ($to_email) {
                //sending email
                if (Mailer::send($from_name, $from_email, $to_email, $subject, $body, true, [], [], $function_tab)) {
                    Acid::sessKill('contact_form');
                    $stats_contact =
                        '<div id="stats_content">' . Acid::executeTpl(SITE_PATH . 'sys/stats/contact.tpl') . '</div>';
