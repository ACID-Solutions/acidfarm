...
class AdminController
{
    ...
    public static function checkAccess($page)
    {
        ...
        elseif ((!User::curLevel(AdminController::$controller[$check_key]['level']))) {
            //Permissions
            $module =
                isset(AdminController::$controller[$check_key]['mod']) ? AdminController::$controller[$check_key]['mod']
                    : null;
            if ($module) {
     ...
     * @param array $config
     */
    public static function addAccess($key, $level, $module = null, $config = [])
    {
        $config['level'] = $level;
     ...
     * @param array $config
     */
    public static function addMenu($key, $parent, $label, $level = 0, $config = [])
    {
        $config['label'] = $label;
        $config['level'] = $level;
     ...
     * @param array $args
     */
    public static function addMethod($key, $function, $args = [])
    {
     ...
     * @param int   $level
     */
    public static function addMenuCat($key, $label, $config = [], $level = 0)
    {
    ...
    public static function medias()
    {
        ...
        Acid::set('admin_title_attr',
            ((array) Acid::get('admin_title_attr') + ['style' => 'color:' . static::color('medias') . ';']));
    ...
    public static function board()
    {
        ...
        $stats['users'] =
            AcidDB::query(
                'SELECT COUNT(*) as count FROM ' . Acid::get('session:table') . ' WHERE `expire` > ' . $expire_date
        ...
        $vars = ['registration' => $registration, 'stats' => $stats, 'lastnews' => $last_news];
    ...
    public static function home()
    {
        $content = Acid::mod('User')
                       ->printAdminBody(SiteConfig::printRemoteHomeForm(),
                           [AcidUrl::build(['page' => 'home']) => Acid::trad('admin_menu_home')]);
    ...
    public static function index()
    {
        ...
        if ($check_key = static::checkAccess($route)) {
            ...
            $module = isset(AdminController::$controller[$check_key]['module'])
                ? AdminController::$controller[$check_key]['module'] : null;
            ...
            if (empty($custom_route)) {
                if (is_callable('static::' . $route)) {
                    ...
                    call_user_func(AdminController::$functions[$check_key]['func'],
                        AdminController::$functions[$check_key]['args']);
                } elseif ($module) {
