...
<div class="<?php echo $o::TBL_NAME; ?>_<?php echo AcidUrl::normalize($key); ?>_filter admin_list_head_filter navbar navbar-default" >

	<?php if ($head_title) { ?>
		<b class="admin_list_head_filter_title navbar-brand" ><?php echo $head_title; ?> : </b>
	<?php } ?>

	<?php if ($showall_title!==false) { ?>
		<a class="<?php echo ($current===null ? 'selected':'unselected'); ?> admin_list_head_filter_all navbar-brand"
	...
	if ($elts = $v['elts']) {
		foreach ($elts as $val=>$label) {
			?>
			<a class="<?php echo ( (isset($current) && ($current==$val)) ? 'selected':'unselected'); ?> navbar-brand"
