...
abstract class AcidModuleCore
{
     ...
     * Si un identifiant est renseigné en entrée depuis $init_id, on initialise l'objet en fonction des valeurs
    ...
    /**
     * Verifie que le module dispose bien d'une constante TBL_NAME et retourne le nom de la table SQL dont il fait
     * référence
     *
    ...
    public static function tblMods($module)
    {
        if ($module::tbl() != $module::tblJoin()) {
            return [$module::tbl(), $module::tblJoin()];
        }
     ...
     * @return string
     */
    public static function currentChange($values = [])
    {
     ...
     * Si $eraseIfUndefined est à true, alors les champs non renseignés dans le tableau en entrée seront initialisés à
    ...
    {
        return Acid::tpl('core/debug.tpl', ['vars' => $this->vars], $this);
    }
    ...
    public function get($key)
    {
        if (isset($this->vars[$key])) {
            ...
            trigger_error('' . get_called_class() . '::get("' . htmlspecialchars($key) . '") val "'
                          . htmlspecialchars($key) . '" doesn\'t exists !', E_USER_WARNING);
        }
    }
    ...
    /**
     * Retourne un tableau accueillant les différentes valeurs du paramétre multi-valeurs (select, radio) renseigné en
     * entrée.
     *
     * @param string $key Nom du paramétre.
     *
    ...
    {
        if (isset($this->vars[$key])) {
            if (in_array(['select', 'radio'], $this->vars[$key]->getFormValOf('type'))) {
     ...
     * @return mixed
     */
    public function getVarForm($key, $print = true, $attr = [], $start = '', $stop = '')
    {
        if (isset($this->vars[$key])) {
    ...
    public function getLabel($key)
    {
        ...
        if (isset($this->config['admin']['head'][$key])) {
            if (is_array($this->config['admin']['head'][$key])) {
                return isset($this->config['admin']['head'][$key]['label'])
                    ? $this->config['admin']['head'][$key]['label'] : $def_label;
            }
     ...
     * Si $id n'est pas trouvé en base de données, alors l'objet est initialisé avec ses valeurs par défaut, on renvoie
     * alors false.
     *
    ...
    {
        $mods = ($mods === null) ? [self::getCLass()] : $mods;
        $current = self::build();
     ...
     *
     * @param mixed   $filter     [ code SQL |
     *                            array(array('field1','operator','value'),array('field2','operator2','value2'),'codeSQL3')
     *                            ]
            ...
            foreach ($filter as $f_val) {
                ...
                if (is_array($f_val) && count($f_val) >= 3) {
                    ...
                    elseif (strtolower($method) === 'in' || strtolower($method) === 'not in') {
                        ...
                        $akey = $add_aquote ? "`" . $key . "`" : $key;
                        $filter_string .= $akey . " " . $method . " '" . ($start !== null ? $start : '')
                                          . addslashes($value) . ($stop !== null ? $stop : '') . "' " . $combo . " ";
                    }
                } //String filter
                else {
                    if (is_string($f_val)) {
                        ...
                        trigger_error('Acid : dbList filter error, invalid param : ' . var_export($f_val, true),
                            E_USER_WARNING);
                    }
                }
            }
     ...
     * @return string
     */
    public static function dbGenerateGroupBy($fields = [])
    {
        $group_by_string = "";
            ...
            if (is_array($select)) {
                ...
                foreach ($select as $sel) {
                    ...
                    if ($key) {
                        $as_str = $as ? " AS '" . $as . "'" : '';
                        $key_str = $func
                            ? $func . '(' . static::dbPref($key, $prefix) . ')'
                            : '' . static::dbPref($key, $prefix)
                              . '';
     ...
     *
     * @param {array} $tables array( array('Table1',array('clé commune')) , array('Table2',array(array('clé tbl1','clé
     *                        tbl2'))) )
     ...
     * @return array | int
     */
    public static function dbGenerateFrom($tables = [], $join = 'LEFT JOIN', $first_as = '')
    {
                                ...
                                foreach ($on as $exp) {
                                    if (is_array($exp)) {
                                        if (count($exp) > 1) {
                                            ...
                                            $b = $exp[1];
                                            $join_table[] =
                                                static::dbPref($a, $first_prefix) . ' = ' . static::dbPref($b, $prefix);
                                        } else {
                                            trigger_error('dbGenerateFrom() : wrong parameters for ' . $tbl . ' join',
                                                E_USER_ERROR);
                                        }
                                    } else {
                                        $join_table[] =
                                            static::dbPref($exp, $first_prefix) . ' = ' . static::dbPref($exp, $prefix);
                                    }
                                }
    ...
    /**
     * Retourne tous les éléments de la table SQL associée à l'objet en fonction de la configuration renseignée par les
     * paramètress en entrée Si $count a la valeur true, retourne seulement le nombre d'élements.
     *
     ...
     * @return array | int
     */
    public static function dbSelList(
        $select = '',
        $filter = '',
        $order = '',
        $limit = '',
        ...
        // Return COUNT()
        if ($count) {
            $req = static::dbGenerateSelect([['*', true, false, 'nb']]) . " " . static::dbGenerateFrom() . " "
                   . $filter_string . $filter_group_by;
            ...
            $req = static::dbGenerateSelect($select) . " " . static::dbGenerateFrom() . " " . $filter_string
                   . $filter_group_by . $order_string . $limit_string;
     ...
     * @return array | int
     */
    public static function dbListMods(
        $mods = [],
        $filter = '',
        $order = '',
        $limit = '',
        ...
        if ($mods) {
            ...
            foreach ($mods_tab as $module => $keys) {
                foreach (Acid::mod($module)->getKeys() as $key) {
                    $select[] = [$key, false, Acid::mod($module)->tblJoin(), Acid::mod($module)->dbPref($key)];
                }
        ...
        // Return COUNT()
        if ($count) {
            $req = static::dbGenerateSelect([['*', true, false, 'nb']]) . " " . static::dbGenerateFrom($from) . " "
                   . $filter_string . " " . $filter_group_by;
            $res = AcidDB::query($req)->fetch(PDO::FETCH_ASSOC);
            ...
            $req = static::dbGenerateSelect($select) . " " . static::dbGenerateFrom($from) . " " . $filter_string
                   . $filter_group_by . $order_string . $limit_string;
    ...
    /**
     * Retourne le nombre d'élements de la table SQL associée à l'objet en fonction de la configuration renseignée par
     * les paramètress en entrée.
     *
     * @param {array | string} $filter Commandes SQL de filtrage, array(arrays([Champs],[Opérateurs],[Valeurs]))
     * @param string $combo Opérateur logique associant les éléments de $filter Valeur par défaut 'AND'
     *
     ...
     * Retourne tous les éléments de la table SQL associée à l'objet en fonction de la configuration renseignée par les
     * paramètresss en entrée Si $count a la valeur true, retourne seulement le nombre d'élements.
     *
     * @param array   $mod_elts Liste des champs prélevés array( string{ [nom table] | [nom table/alias] } =>
     *                          string|array [champs prélevés] )
     * @param array   $mod_rel  Liste des relations array( string [alias table] => array(
     *                          [alias1]=>[champ1],[alias2]=>[champs2] ) )
     * @param {array | string} $filter Commandes SQL de filtrage, array(arrays([Champs],[Opérateurs],[Valeurs]))
     * @param {array | string} $order Commandes d'indexation SQL, array(arrays( [Champs]=>{true : DESC | false : ASC}
     *                          ))
     * @param {int | string} $limit Commande de limite SQL
     ...
     * @return array | int
     */
    public static function dbJoinedList(
        $mod_elts,
        $mod_rel,
        $filter = '',
        $order = '',
        ...
        if (!is_array($filter)) {
            ...
            foreach ($filter as $f_val) {
                if (is_array($f_val) && count($f_val) >= 4) {
                    ...
                    if (strtolower($method) === 'in' || strtolower($method) === 'not in') {
                        ...
                        if (count($in_vals)) {
                            $filter_string .= "`" . $mod . "`.`" . $key . "` " . $method . " (" . implode(',', $in_vals)
                                              . ") " . $combo . " ";
                        } else {
                            $filter_string .= ((strtolower($method) === 'in') ? " 0 " : " 1 ") . $combo . " ";
                        }
                    } else {
                        $filter_string .= "`" . $mod . "`.`" . $key . "` " . $method . " '" . $start
                                          . addslashes($value) . $stop . "' " . $combo . " ";
                    }
                } else {
                    if (is_string($f_val)) {
                        $filter_string .= " " . $f_val . " " . $combo;
            ...
            $filter_string =
                empty($filter_string) ? '' : "WHERE " . substr($filter_string, 0, -strlen($combo) - 1) . " ";
        }
    ...
    /**
     * Joint plusieures tables
     * Retourne le nombre d'éléments de la table SQL associée à l'objet en fonction de la configuration renseignée par
     * les paramètress en entrée
     *
     ...
     *
     * @param array   $tab
     * @param boolean $limit si vrai, retourne le premier élément parmis ceux trouvés, sinon retourne une valeur que si
     *                       cette dernière est unique
     * @param array   $order
     *
     * @return boolean
     */
    public function dbInitSearch($tab, $limit = false, $order = [])
    {
        $filter = [];
    ...
    {
        $force =
            $force !== null ? $force : (Acid::exists('modcache:force_time') ? Acid::get('modcache:force_time') : false);
    ...
    public function updateCacheTime($value = null)
    {
        if (isset($this->vars[static::cacheTimeKey()])) {
            ...
            $changes = $this->initVars([static::cacheTimeKey() => $value]);
    ...
    public static function genUrlKey($key, $url = null, $format = null, $cache_time = null)
    {
        ...
        if (isset($module->vars[$key])) {
            ...
            if ($cache_time) {
                $cachetimekey =
                    Acid::get('modcache:key_time_name') ? Acid::get('modcache:key_time_name') : 'cache_time';
    ...
    public function exeUploadKey($key, $val, $name = null)
    {
        ...
        $filename = '';
        if ($this->vars[$key]->uploadProcess($this->getId(), $key, $filename, [], $vals)) {
            $this->dbUpdate([$key]);
        }
    }
     ...
     * @param array  $config
     */
    public function postConfigure($vals = [], $do = null, $config = null)
    {
        if ($config !== null) {
     ...
     * @return boolean
     */
    public function postSuccess($vals = [], $do = null)
    {
        //cache time
        if (in_array($do, ['add', 'update'])) {
    ...
    public function postRemove($id = null, $dialog = null)
    {
        ...
        if ($this->dbCount([[static::tblId(), '=', $id]])) {
            $class = get_called_class();
            $obj = new $class($id);
            ...
            if ($success) {
                ...
                if ($obj->postSuccess([static::tblId() => $id], 'remove')) {
                    return $obj;
                } else {
                    return false;
                }
            ...
            if ($upkeys = $this->getUploadVars()) {
                foreach (array_keys($upkeys) as $uk) {
                    if (isset($acl_key_comp[$uk])) {
                        ...
                        $acl_key_comp['tmp_name_' . $uk] =
                            isset($acl_key_comp['tmp_name_' . $uk]) ?: $acl_key_comp[$uk];
                    }
                }
            }
        }
     ...
     * Retourne un tableau avec uniquement les éléments dont l'accés est autorisé, ou false si l'utilisateur n'a aucun
     * droit.
     *
     * @param string $do
     * @param array  $vals
    ...
    public function getUserPermission($do = null, $type = null, $id_user = null)
    {
        ...
        $types = $type ? [$type] : Acid::get('permission_groups');
        
        $def_do = Acid::exists('permission:' . static::TBL_NAME) ? Acid::get('permission:' . static::TBL_NAME) : [];
        $to_do = $do ? [$do] : array_keys($def_do);
                    ...
                    case 'id_group' :
                        ...
                        foreach ($user->getGroups() as $idg) {
                            if (in_array($idg, $permission) || in_array($idg, $permission_def)) {
                                Acid::log('permission',
                                    get_called_class() . '::getUserPermission() ' . $d . '=> ' . $t . ' permission');
        ...
        if (static::getPostDo()) {
            ...
            switch (static::getPostDo()) {
                ...
                default :
                    $do = static::getPostDo();
                    $search = [' ', '_', '-'];
                    $methode_name = 'post' . str_replace($search, '', ucwords($do, implode('', $search)));
                    if (method_exists($this, $methode_name)) {
                        if ($vals = $this->postACL($do, $_POST)) {
                            if ($vals = $this->checkVals($vals, $do)) {
                                if (isset($_POST[static::tblId()])) {
                                    $vals[static::tblId()] = $_POST[static::tblId()];
                                }
                                
                                return call_user_func_array([$this, $methode_name], [$vals]);
                            }
                        }
                    }
                    break;
     ...
     * @param array $config
     */
    public function restGet($config = [])
    {
     ...
     * @param array $config
     */
    public function restList($config = [])
    {
        if (!empty($this->config['rest']['active'])) {
            $def_filter = isset($this->vars['active']) ? [['active', '=', 1]] : [];
    ...
    /**
     * Retourne la valeur de configuration de l'objet qui renseignée en entrée lorsque celle-ci existe, renvoie null
     * sinon.
     *
    ...
    public function getIconLink($link = null, $image = null, $title = 'bouton', $click = null, $tpl = null)
    {
        ...
        return Acid::tpl($tpl, ['link' => $link, 'image' => $image, 'title' => $title, 'click' => $click], $this);
    }
     ...
     * @return array
     */
    public function buildAdminOnglets($name, $builder = [], $excluder = [], $key = null, $src = null)
    {
        $url = AcidUrl::build($builder, $excluder, $src);
        $sel = $builder;
        
        if ((empty($builder)) && in_array(static::preKey('do'), $excluder)) {
            $sel = [static::preKey('do') => $this->getDefaultDo()];
        }
        
        $onglet = ['url' => $url, 'name' => $name, 'selector' => $sel];
     ...
     * @return array
     */
    public function getStandardOnglets($config = ['list', 'add', 'search'])
    {
        $tab = [];
        ...
        foreach ($config as $key) {
            switch ($key) {
                case 'list' :
                    $tab[] = $this->buildAdminOnglets(Acid::trad('admin_onglet_list'), [static::preKey('do') => 'list'],
                        [static::preKey('id')], 'list');
                    break;
                case 'add' :
                    $tab[] =
                        $this->buildAdminOnglets(Acid::trad('admin_onglet_add'), [static::preKey('do') => 'add'], [],
                            'add');
                    break;
                case 'search' :
                    $builder = array_merge($this->getAdminCurNav(), [static::preKey('do') => 'search']);
                    $tab[] = $this->buildAdminOnglets(Acid::trad('admin_onglet_search'), $builder, [], 'search');
                    break;
            }
        }
    ...
    {
        return isset($this->config['admin']['list']['disable_actions'])
            ? $this->config['admin']['list']['disable_actions'] : false;
    }
    ...
    public function getStandardActionPrint($elt)
    {
        ...
        return [
            'link'  => AcidUrl::build([static::preKey('do') => 'print', static::preKey('id') => $ident]),
    ...
    public function getStandardActionUpdate($elt)
    {
        ...
        return [
            'link'  => AcidUrl::build([static::preKey('do') => 'update', static::preKey('id') => $ident]),
    ...
    {
        $ident = isset($elt[static::tblId()]) ? $elt[static::tblId()] : $elt[self::dbPref(static::tblId())];
        $next = isset($this->config['admin']['remove']['next_page']) ? $this->config['admin']['remove']['next_page']
            : AcidUrl::build($this->getAdminCurNav());
        $next = $next ? $next : '';
        $del_form = Acid::tpl('admin/admin-form-delete.tpl', ['id' => $ident, 'next' => $next], $this);
        
        return [
            'link'   => '#',
            'image'  => Acid::themeUrl('img/admin/btn_supprimer.png'),
            'title'  => Acid::trad('admin_action_remove'),
            'click'  => 'if (confirm(\'Supprimer ?\')){window.document.getElementById(\'' . static::preKey($ident)
                        . '_delform\').submit()};return false;',
            'script' => $del_form
        ];
    }
     ...
     * @return string
     */
    public function getStandardActionTab($elt, $config = ['print', 'update', 'delete'])
    {
        $ident = isset($elt[static::tblId()]) ? $elt[static::tblId()] : $elt[self::dbPref(static::tblId())];
     ...
     * @return string
     */
    public function printAdminActionTab($links = [], $forms = [], $elt = [], $conf = [])
    {
        ...
        return Acid::tpl($tpl, ['actions' => $actions], $this);
    }
     ...
     * @return string
     */
    public function getAdminTh($cont, $attr = [], $conf = [])
    {
        $tpl = isset($conf['tpl']['th']) ? $conf['tpl']['th'] : 'core/tab/admin-th.tpl';
        
        return Acid::tpl($tpl, ['cont' => $cont, 'attr' => $attr], $this);
    }
     ...
     * @return string
     */
    public function getAdminTd($cont, $attr = [], $conf = [])
    {
     ...
     * @return string
     */
    public function getAdminTr($line, $class = null, $attr = [], $conf = [])
    {
        if ($class) {
        ...
        return Acid::tpl($tpl, ['line' => $line, 'attr' => $attr], $this);
    }
    
    /**
     * Renvoie un tableau associatif des données en basse poru ce module avec pour couple clé => valeur, le nom de
     * champs du module passé en argument. Si les deux champs sont vides, cette méthode ira voir la présence de
     * config['assoc:index'] && config['assoc:value'] qui devront être renseigné dans le module correspondant.
     *
     * @param string $key_val   Le nom du champ du module qui doit servir de valeur pour le tableau associatif (ex:
     *                          title)
     * @param string $key_index Le nom du champ du module qui doit servir de clé pour le tableau associatif (ex:
     *                          id_module)
     ...
     */
    public static function getAssoc($key_val = null, $key_index = null, $hsc_if_value = true, $order = [], $filter = [])
    {
        ...
        if (!$key_val) {
            throw new Exception(get_called_class()
                                . "::getAssoc() - config['assoc:value'] == null, association impossible.");
        }
     ...
     * @param string  $key_index Le nom du champ du module qui doit servir de clé pour le tableau associatif (ex:
     *                           id_module)
     * @param string  $key_value Le nom du champ du module qui doit servir de valeur pour le tableau associatif (ex:
     *                           title)
        ...
        if ($tab) {
            foreach ($tab as $elt) {
                $value = ($key_value === null)
                    ? $elt : ($hsc_if_value ? htmlspecialchars($elt[$key_value]) : $elt[$key_value]);
     ...
     * @return string
     */
    public function printAdminTab($cols = [], $rows = null, $conf = [])
    {
        $my_rows = [];
        foreach ($rows as $l_tab) {
            if (is_array($l_tab)) {
                ...
                foreach ($l_tab as $k => $td) {
                    $line .= $this->getAdminTd($td, ['class' => str_replace('.', '_', 'col_' . $k)], $conf) . "\n";
                }
        ...
        return Acid::tpl($tpl, ['cols' => $cols, 'rows' => $my_rows, 'config' => $conf], $this);
    }
    ...
    public function printAdminOnglets($onglets = [], $config = [])
    {
        ...
        if ($onglets) {
            foreach ($onglets as $url_onglet => $conf_onglet) {
                ...
                $_onglet->selector =
                    is_array($conf_onglet) ? (isset($conf_onglet['selector']) ? $conf_onglet['selector']
                        : $_onglet->url) : $url_onglet;
     ...
     * @return string
     */
    public function printAdminBody($content, $onglets = [], $title = null, $title_attr = null)
    {
        $this->adminNav();
        ...
        return Acid::tpl('admin/admin-body.tpl',
            ['menu' => $menu, 'title' => $title, 'title_attr' => $title_attr, 'content' => $content]);
    }
     ...
     * @return string
     */
    public function exeAdminController($do, $config = [])
    {
        $class = get_called_class();
        $keys = ['add', 'search', 'update', 'print', 'list'];
        ...
        switch ($do) {
            ...
            default :
                $config_list = isset($config['list']) ? $config['list'] : [];
        ...
        if (empty($this->config['no_more_onglets'])) {
            switch ($do) {
                case 'update' :
                    ...
                    $this->config['onglets'][$do][] = $this->buildAdminOnglets($name, [$this->preKey('do') => $do]);
                    break;
     ...
     * @return string
     */
    public function printAdminInterface($config = [])
    {
        $onglets = (isset($config['onglets'])) ? $config['onglets'] : [];
     ...
     *                        Il est possible de lui renseigner $=>'true' en troisieme paramètre pour utiliser
     *                        ${$cuf[0]} en entête de fonction
     * @param array $cup      Paramètre 2 de la function  call_user_func_array de PHP
     *
     * @return array
     */
    public function callFunction($cuf, $cup = [])
    {
        if ($cuf[0] == '$') {
            if ($cuf[1] != 'this') {
                $cuf[1] = ${$cuf[1]};
    ...
    /**
     * Retourne les paramètres de filtrage pour l'administration du module sous forme d'un couple ([filtres],[paramètre
     * d'indexation])
     *
    ...
    public function getAdminListOptions($mods = false)
    {
        ...
        foreach ($gkeys as $key) {
            if (isset($gets['fm_' . $key]) && isset($gets['fv_' . $key]) && strlen($gets['fv_' . $key])) {
                ...
                switch ($gets['fm_' . $key]) {
                    case 'contain' :
                        $filter[] = [$key_filter, 'LIKE', $search_val, '%', '%'];
                        break;
                    case 'start' :
                        $filter[] = [$key_filter, 'LIKE', $search_val, '', '%'];
                        break;
        ...
        if (isset($gets['lo']) && isset($gets['ld']) && $valid_key) {
            if (isset($this->config['admin']['list']['orderval'][$gets['lo']])) {
                $order = [
                    '()' => $this->config['admin']['list']['orderval'][$gets['lo']] . ' ' . ($gets['ld'] ? 'DESC'
                            : 'ASC')
                ];
            } else {
                $order = [$gets['lo'] => ($gets['ld'] ? true : false)];
            }
     ...
     * @return string
     */
    public function printAdminList($conf = [])
    {
        $this->printAdminConfigure('list', $conf);
        
        //Gestion des filtres additionnels
        $add_filter = isset($conf['filter']) ? $conf['filter'] : [];
        $add_order = isset($conf['order']) ? $conf['order'] : [];
        $sub_order = isset($conf['sub_order']) ? $conf['sub_order'] : [];
        ...
        if ($mods) {
            $modules = [$this->getClass() => $this->getClass()];
     ...
     * @return string
     */
    public function printAdminListBody($content, $infos, $nav, $conf = [])
    {
        ...
        return Acid::tpl($tpl, ['infos' => $infos, 'nav' => $nav, 'content' => $content, 'head_links' => $head_links],
            $this);
    }
     ...
     * @return string
     */
    public function printAdminListHeadBtn($conf = [])
    {
        ...
        return Acid::tpl($tpl, ['links' => $links], $this);
    }
     ...
     * @return string
     */
    public function genAdminListHeadFilter($conf = [])
    {
        $content = '';
        
        $def_filter_head =
            isset($this->config['admin']['list']['head_filters']) ? $this->config['admin']['list']['head_filters'] : [];
        ...
        if (!empty($filter_head)) {
            foreach ($filter_head as $key => $elts) {
                ...
                $content .= Acid::tpl('admin/admin-list-filter.tpl',
                    ['current' => $current, 'elts' => $elts, 'key' => $key], $this);
            }
        }
     ...
     * @return array
     */
    public function genAdminListHead($conf = [])
    {
        //initialisation
        $cur_nav = $this->getAdminCurNav();
        $ki = static::preKey();
        ...
        foreach ($t_champs as $key) {
            ...
            $f = array_merge($cur_nav, [$ki . 'lo' => $key, $ki . 'ld' => $ld]);
            ...
            if (isset($this->config['admin']['head'][$key]['func'])) {
                ...
                if (is_array($specs)) {
                    $args = isset($specs['args']) ? $specs['args'] : ['__VAL__'];
            ...
            $url = isset($this->config['admin']['head'][$key]['url']) ? $this->config['admin']['head'][$key]['url']
                : AcidUrl::build($f);
            
            $th_val = ['url' => $url, 'name' => $value, 'key' => $key];
     ...
     * @return array
     */
    public function genAdminListLine($fields, $elt, $conf = [])
    {
        $line = [];
     ...
     * @return string
     */
    public function genAdminListAction($elt, $conf = [])
    {
        $config_btn =
            isset($this->config['admin']['list']['actions']) ? $this->config['admin']['list']['actions'] : null;
     ...
     * @return string
     */
    public function genAdminListFormat($elts, $conf = [])
    {
        return $elts;
    }
     ...
     * @return string
     */
    public function genAdminListLineCustomClass($elt, $conf = [])
    {
        return '';
    }
    
    /**
     * Formate pour le listing d'administration du module les éléments en entrée sous forme d'une chaîne de caractères
     * mise en forme.
     *
     ...
     * @return array
     */
    public function genAdminListContent($elts, $conf = [])
    {
        ...
        if (!$disable_action) {
            $th_tab['std_actions'] =
                ['url' => false, 'name' => Acid::trad('admin_list_btns_label'), 'key' => 'std_actions'];
        }
    ...
    {
        return isset($this->config['admin'][$do]['tpl']) ? $this->config['admin'][$do]['tpl'] : [];
    }
     ...
     */
    public function getAdminListParams($nb_elts, $conf = [])
    {
        $cur_nav = $this->getAdminCurNav();
        
        $def_tpl = isset($this->config['admin']['list']['tpl']['pagination'])
            ? $this->config['admin']['list']['tpl']['pagination'] : null;
        $tpl = isset($conf['tpl']['pagination']) ? $conf['tpl']['pagination'] : $def_tpl;
        
        $ll = isset($this->admin_nav['ll'])
            ? $this->admin_nav['ll']
            : (
        ...
        $start = AcidUrl::build($cur_nav, [static::preKey('lp')]);
        $middle = '&amp;' . static::preKey('lp') . '=';
        
        $nav = AcidPagination::getNav($page, $nb_elts, $ll, $tpl, ['link_start' => $start, 'link_middle' => $middle]);
        
        $limit = $limit = ($ll * ($page - 1)) . ',' . $ll;
        
        return [$ll, $limit, $page, $nav];
    }
     ...
     * @return array  list( nombre maxi d'éléments par page , limite d'affichage, page courante, pagination )
     */
    public function printAdminListInfo($nb_elts, $page, $ll, $hide_filter_ind, $conf = [])
    {
        //$this->getAdminCurNav();
        $cur_nav = $this->getAdminCurNav();
        ...
        $txt_info = Acid::trad('admin_list_total_elts',
            ['__TOTAL__' => $nb_elts, '__NB__' => $aff, '__START__' => $start, '__STOP__' => $stop]);
        $vars = [
    ...
    {
        $this->config['print'] = [];
        if ($bools = $this->getVarsByType(['AcidVarBool'])) {
            foreach ($bools as $key => $var) {
                $this->config['print'][$key] = ['type' => 'bool'];
            }
        }
        
        if ($list = $this->getVarsByType(['AcidVarList', 'AcidVarRadio'])) {
            foreach ($list as $key => $var) {
                $this->config['print'][$key] = ['type' => 'tab', 'tab' => $var->getElts()];
            }
        }
    }
    ...
    public function printAdminForm($do)
    {
        ...
        $lang_keys = isset($this->config['multilingual']['keys']) ? $this->config['multilingual']['keys']
            : $this->getMultiLingualKeys();
        
        $images_keys = array_keys($this->getVarsImages());
        $files_keys = array_keys($this->getVarsFiles());
        
        $excluded_keys =
            isset($this->config['admin'][$do]['keys_excluded']) ? $this->config['admin'][$do]['keys_excluded'] : [];
        ...
        if ($next_page === null) {
            $next_page = ($do != 'add') ? false : AcidUrl::build([$this->preKey() . 'do' => 'list']);
        }
            ...
            foreach ($keys as $key) {
                if (isset($this->vars[$key])) {
                    if (!in_array($key, $excluded_keys)) {
                        $params = isset($this->config['admin'][$do]['params'][$key])
                            ? $this->config['admin'][$do]['params'][$key] : [];
                        $start = isset($this->config['admin'][$do]['start'][$key])
                            ? $this->config['admin'][$do]['start'][$key] : '';
                        $stop =
                            isset($this->config['admin'][$do]['stop'][$key]) ? $this->config['admin'][$do]['stop'][$key]
                                : '';
                        $body_attrs = isset($this->config['admin'][$do]['body_attrs'][$key])
                            ? $this->config['admin'][$do]['body_attrs'][$key] : [];
                        ...
                        if (in_array($key, $lang_keys)) {
                            ...
                            $body_attrs['class'] =
                                !empty($body_attrs['class']) ? $body_attrs['class'] . ' ' . $l_class : $l_class;
                        }
                        ...
                        if (Conf::get('plupload:active')) {
                            if (Conf::get('plupload:all') || Conf::get('plupload:key:' . $key)) {
                                if (in_array($key, array_keys($this->getUploadVars()))) {
                                    $params['class'] =
                                        isset($params['class']) ? $params['class'] . ' plupload_for_' . $key
                                            : 'plupload_for_' . $key;
                                    $as = !isset($this->config['plupload']['autosubmit'])
                                        ? Acid::get('plupload:autosubmit') : $this->config['plupload']['autosubmit'];
                                    $multi = !empty($this->config['plupload']['multi'][$key]);
                                    
                                    if ($multi) {
                                        $stop .= '<div class="plupload_multi_for_' . $key . '"></div>';
                                        Conf::add('plupload:multi', [
                                            'selector'    => '.' . $this::TBL_NAME . '.' . $this->preKey($do)
                                                             . ' .plupload_multi_for_' . $key,
                                            'extensions'  => implode(',', $this->vars[$key]->getConfig('ext')),
                                            'hide_submit' => $as, 'key' => $key
                                        ]);
                                    } else {
                                        Conf::add('plupload:selectors', [
                                            '.plupload_for_' . $key, '.' . $this::TBL_NAME . '.' . $this->preKey($do),
                                            $this->vars[$key]->getConfig('ext'), $as
                                        ]);
                                    }
                                }
                            }
                    ...
                    trigger_error(get_called_class() . '::printAdminForm(' . $do . ') key "' . $key
    ...
    {
        $default =
            isset($this->config['multilingual']['flags']['default']) ? $this->config['multilingual']['flags']['default']
                : false;
        $current =
            isset($this->config['multilingual']['flags'][$do]) ? $this->config['multilingual']['flags'][$do] : $default;
        ...
        if ($current) {
            ...
            return Acid::tpl($tpl_flag, ['def_lang' => $cur, 'langs' => $langs, 'do' => $do, 'keys' => $keys]);
        }
    }
    ...
    public function printAdminElt()
    {
        ...
        return Acid::tpl('admin/admin-print.tpl', ['content' => $tbl->html()], $this);
    }
    ...
    public function printAdminSearch()
    {
        ...
        return Acid::tpl($tpl, [
            'admin_nav'   => $admin_nav, 'll' => $ll, 'vars' => $this->vars, 'keys' => $keys, 'get' => $_GET,
    ...
    public function printFormToggle($key, $obj = null, $ajax = false)
    {
        ...
        $form = new AcidForm('post', '');
        $form->setFormParams(['class' => 'toggle_form to_value_' . $new_state . ' ' . $ident]);
        ...
        if ($ajax) {
            $onclick = "$('." . $ident . "').find('[type=submit]').attr('disabled','disabled'); " .
                       ...
                       "if (res.success) { " .
                       "var nval = res.obj." . $real_key . " ? 0 : 1 ; var nlab = !nval ? '" . Acid::trad('yes') . "':'"
                       . Acid::trad('no') . "'; " .
            ...
            $form->setFormParams(['onsubmit' => $onclick]);
        }
     ...
     * @return string
     */
    public function printFormQuickChange($key, $obj = null, $ajax = false, $params = [])
    {
        $obj = $obj === null ? $this : $obj;
        
        $ident = 'form_quickchange_' . $obj->checkTbl() . '_' . $obj->getId() . '_' . $key;
        $form = new AcidForm('post', '');
        $form->setFormParams(['class' => 'quickchange_form ' . $ident]);
        ...
        if ($ajax) {
            $loader = htmlspecialchars('<img style="margin-left:30px; display:inline; vertical-align:middle;" src="'
                                       . Acid::themeUrl('img/admin/loading.gif') . '" alt="..." title="'
                                       . Acid::trad('loading') . '" />');
            $onclick =
        ...
        $form->addSubmit('', $label, ['style' => 'display:none;']);
     ...
     * @param bool  $printed utilise getPrined pour l'affichage des valeurs (attention, printAdminConfigureCSV
     *                       ré-initialise $this->config)
     *
     * @return AcidCSV
     */
    public function exportCSV($filter = '', $fields = [], $config = null, $label = false, $printed = true)
    {
    ...
    public function getPrintedAssoc($key = "")
    {
        ...
        $tab = [
            'AcidVarBool'     => ['type' => 'bool'],
            'AcidVarDateTime' => ['type' => 'date', 'format' => 'datetime', 'empty_val' => '-'],
            'AcidVarDate'     => ['type' => 'date', 'format' => 'date', 'empty_val' => '-'],
            'AcidVarUrl'      => ['type' => 'link', 'absolute' => true],
            'AcidVarEmail'    => ['type' => 'mailto'],
            'AcidVarText'     => ['type' => 'split'],
            'AcidVarTime'     => ['type' => 'time'],
            'AcidVarImage'    => ['type' => 'img', 'link' => 'src', 'size' => ($admin_format ? $admin_format : 'mini')],
            'AcidVarList'     => ['type' => 'tab', 'tab' => $this->vars[$key]->getElts()],
            'AcidVarFile'     => ['type' => 'link', 'absolute' => false, 'basename' => true, 'popup' => true]
        ];
     ...
     * @return mixed
     */
    public function getPrinted($key, $elt = null, $conf = [])
    {
        $nkey = isset($this->vars[$key])
            ? $key : (isset($this->vars[self::dbPrefRemove($key)]) ? self::dbPrefRemove($key) : $key);
            ...
            if (isset($specs['type'])) {
                switch ($specs['type']) {
                    case 'link' :
                        ...
                        return Acid::tpl('core/print/link.tpl', ['url' => $url_val, 'text' => $text, 'popup' => $popup],
                            $this);
                        break;
                    ...
                    case 'mailto' :
                        ...
                        return Acid::tpl('core/print/mailto.tpl', ['email' => $val, 'text' => $text], $this);
                        break;
                    ...
                    case 'quickchange' :
                        ...
                        $params = isset($specs['params']) ? $specs['params'] : [];
                    ...
                    case 'date' :
                        ...
                        $format =
                            $format == 'datetime_small' ? Acid::get('date_format:datetime_small', 'lang') : $format;
                    ...
                    case 'img' :
                        ...
                        return Acid::tpl('core/print/img.tpl',
                            ['size' => $size, 'view' => $view, 'src' => $val, 'key' => $src_key], $this);
                        break;
                        ...
                        if (isset($specs['name']) && isset($specs['args'])) {
                            ...
                            foreach ($specs['args'] as $k => $v) {
                                if ($specs['args'][$k] === '__ELT__') {
                                    ...
                                    $specs['args'][$k] = isset($conf['format_elts']) ? $conf['format_elts'] : [];
                                } else {
