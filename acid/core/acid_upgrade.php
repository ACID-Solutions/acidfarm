...
class AcidUpgrade {
	...
	public function __construct($type='upgrade') {
		...
		$this->exclued = array('files'=>Acid::get('upgrade:exclued:files'),'folders'=>Acid::get('upgrade:exclued:folders'));
		$this->folders = array();
	...
	public function setFolders($version=null) {
		...
		while (($file = readdir($h)) !== false) {
			if ( ($file != '.') && ($file != '..') && (is_dir($this->path.$file)) ) {
				if (!in_array($file,$this->exclued['folders'])) {
					if ($version < $file) {
		...
		foreach ($this->folders as $folder) {
			...
			while (($file = readdir($h)) !== false) {
				...
				if ( (!is_dir($cur_path.$file)) && (in_array($cur_ext,$ext)) ) {
					if (!in_array($file,$this->exclued['files'])) {
