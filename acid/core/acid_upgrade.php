...
class AcidUpgrade
{
    ...
    public function __construct($type = 'upgrade')
    {
        ...
        $this->excluded = [
            'files' => Acid::get('upgrade:excluded:files'), 'folders' => Acid::get('upgrade:excluded:folders'),
            'dotfiles' => Acid::get('upgrade:excluded:dotfiles')
        ];
        $this->folders = [];
        $this->files = ['sql' => [], 'php' => [], 'sh' => []];
    }
    ...
    public function setCurVersion($version)
    {
        ...
        if (is_dir($this->path . $version)) {
            file_put_contents($this->path . $version . '/.done', date('Y-m-d H:i:s'));
        }
    ...
    {
        if ($check_until = $this->checkCurVersion()) {
            $this->setFolders(0, $check_until);
            
            $missing = [];
            foreach ($this->folders as $dir) {
                if (!file_exists($this->path . $dir . '/.done')) {
                    $missing[] = $dir;
                }
            }
     ...
     * @param string $version (YYYY-mm-dd-x)
     */
    public function setFolders($version = null, $last_version = null)
    {
        $version = self::initVersion($version);
        $last_version = $last_version !== null ? $last_version : $this->checkLastVersion();
    ...
    public function upgrade($version = null)
    {
        ...
        foreach ($this->folders as $folder) {
            ...
            if (empty($this->files['sh'])) {
                if (
                    ($this->mode == 'dev')
                    && ((!empty($this->files['common'])) || (!empty($this->files['sql']))
                        || (!empty($this->files['php'])))
                    && (empty($_POST[$this->process_name]))
                ) {
                    $label = (($this->type == 'update') || ($this->type == 'upgrade'))
                        ? $this->type : 'update of ' . $this->type;
                    $input_label = (($this->type == 'update') || ($this->type == 'upgrade'))
                        ? $this->type : 'update ' . $this->type;
                    echo nl2br(
                ...
                foreach ($this->files['common'] as $common_file) {
                    ...
                    switch ($cur_ext) {
                        case 'sql' :
                            $this->treatSQL($common_file);
                            break;
                        case 'php' :
                            $this->treatPhp($common_file);
                            break;
                    }
                }
                ...
                */
                ...
                echo 'A new  version of ' . $this->type .
                     ' is available and need to be installed manually ( ' . $folder. ' )';
                exit();
            }
        }
    }
    ...
    {
        if ($this->mode != 'off') {
            if ($folders = $this->getMissing()) {
                if ($folders !== true) {
                    var_dump($folders);
                    echo 'Some updates are missing but should have been done ( ' . implode(', ', $folders) . ' )';
